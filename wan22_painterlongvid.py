...
import node_helpers  # kept for potential future use, harmless
import nodes


class FossielPainterLongVideoTiled:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    def execute(
        ...
        if has_start or has_end:
            ...
            elif has_prev:  # no start, but has previous_video → use its last frame
            ...
            if has_prev and previous_video.shape[0] >= 2:
                ...
                ref_motion_latent_temp = vae.encode_tiled(
                    ref_motion[:, :, :, :3],
            ...
            # only previous_video branch
            last_frame = previous_video[-1:].clone()
            last_frame_resized = comfy.utils.common_upscale(
                last_frame.movedim(-1, 1), width, height, "bilinear", "center"
            ).movedim(1, -1)

            image_seq = torch.full((length, height, width, 3), 0.5, device=device, dtype=torch.float32)
            ...
            mask = torch.ones((1, 1, latent_timesteps, height // 8, width // 8), device=device)
            mask[:, :, 0] = 0.0
            ...
            ref_motion_latent_temp = vae.encode_tiled(
                ref_motion_resized[:, :, :, :3],
                tile_x=tile_size, tile_y=tile_size, overlap=overlap,
                tile_t=temporal_size, overlap_t=temporal_overlap
            )
            ref_motion_latent = ref_motion_latent_temp[:, :, -19:]

        # reference_latents (also tiled)
        ref_latents = []
        ...
        last_latent = vae.encode_tiled(last_frame_for_ref[:, :, :, :3],
                                       tile_x=tile_size, tile_y=tile_size, overlap=overlap)
        ...
        if initial_reference_image is not None:
            ...
            init_latent = vae.encode_tiled(init_img_resized[:, :, :, :3],
                                           tile_x=tile_size, tile_y=tile_size, overlap=overlap)
            ref_latents.append(init_latent)

        # conditioning injection (unchanged)
        ...
        shared_values = {"concat_latent_image": concat_latent_image, "concat_mask": mask}
        ...
        return (pos_out, neg_out, {"samples": latent})
